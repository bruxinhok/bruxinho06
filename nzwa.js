...
async function starts() {
	...
	nzwa.on('chat-update', async (mek) => {
			...
			mess = {
				...
				only: {
					...
					Badmin: '[❕] Este comando só pode ser usado quando o bot se torna administrador!',
                                        daftarB: `──「 AINDA NAO REGISTRADO 」──\nSlv irmao!\nVoce ainda nao esta registrado no banco de dados do Bruxinho \n\nComando : ${prefix}register nome|idade\nExemplo : ${prefix}register Bruxinho|19`,
				}
			}
		...
		reply(`Link Detectado desculpe ${sender.split("@")[0]} voce sera expulso deste grupo em 5segundos`)
		setTimeout( () => {
	    ...
	    case 'kapankah':
			...
			case 'closegc':
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Cancao Encontrada!!!*\nTitulo : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE NOVAMENTE ENVIADO POR FAVOR, NAO ENVIE SPAM BY BRUXINHO MODS*`
